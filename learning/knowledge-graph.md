...
| `guards-for-impossible-states` | `seed` | `truthiness-and-guards` | Wrote `if (!value.isFinite(x)) return value;` in `roundForDisplay`, commented "in theory should never be not a number, but why not just have this condition here anyway" — defensive code for a state `evaluate()` already rules out, and itself broken two ways (`x` undeclared, `isFinite` not a method on numbers). Deleted it when asked where the guarantee actually lives, but never stated the reason. | 2026-08-31 |
