...
| `clearAll()` + the four-branch click handler | `known` | Learner-written. `clearAll` is the single definition of "fresh". The handler routes digits, `.clear`, `.operator`, and `.equals`, each with an early return. `=` bails when no operator has been chosen. → [[truthiness-and-guards]], [[calculator-state-machine]] |
| `let firstNum / secondNum / operator` (lines 1–3) | `known` | All three now written by the handler and reset by `clearAll`. `operator` holds the *name* of the operation, never the function — that separation is deliberate and was learned by breaking it. → [[calculator-state-machine]] |
...
| `getOperation(name)` | `known` | Turns a `data-op` string into the function it names — the bridge between what a button says and what `operate` needs. Function shell agent-written after the learner got stuck on `return` scope; all four cases learner-written. → [[operator-string-to-function]], [[return-scope]] |
