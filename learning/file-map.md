...
Last updated: 2026-09-01 (through task 8.3)
...
| `data-key` on the digit and `.` buttons | `known` | **Added 8.3.** The character a user would type to reach that button. Deliberately separate from the button's label even where the two match today — in 8.4 they diverge (`÷` is typed `/`, `=` is typed `Enter`). → [[data-attributes]] |
