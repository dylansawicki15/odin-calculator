...
| the `keydown` listener on `document` | `known` | **Learner-written 2026-09-01, diagnostic only so far** — it logs `event.key` and nothing else. On `document` rather than `.buttons` because the container is never focusable; `event.key` rather than the physical-position property so numpad and number-row keys arrive identically. Two facts it surfaced that 8.3/8.4 have to handle: holding a key repeats the event, and every modifier and function key fires one too — so routing must be a lookup that ignores the unrecognised, not a chain that assumes. → [[keyboard-events]], [[event-listeners]] |
